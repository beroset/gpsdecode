...
public:
    ...
    static constexpr myfloat scalefactor{180.0 * epsilon};
private:
    myfloat a;
};
 ...
 * +--- 1 = S, 0 = N latitude
 *
 * Latitude and longitude are scaled such that a
 * 90 degree value is 0x10'0000
 *
 * range of latitude is [0, 90) with S acting as sign bit
 * range of longitude is [0, 180) with W acting as sign bit
 * range of color = [0, 32)
 *
...
struct WANAddr : public std::array<uint8_t, 6> {
    ...
    constexpr Coord getLat() const {
        ...
        return Coord{(South ? 0 : 90) - mylat * Coord::scalefactor};
    }
    ...
    constexpr Coord getLong() const {
        ...
        return Coord{mylong * Coord::scalefactor - (West ? 180 : 0)};
    }
...
public:
    ...
    constexpr WANAddr toWANAddr() const {
        ...
        uint32_t enc_lat = ((latitude.toFloat() < 0 ? 0.0 : 90.0) - latitude.toFloat() + Coord::epsilon) / Coord::scalefactor;
        uint32_t enc_long = ((longitude.toFloat() < 0 ? 180.0 : 0.0) + longitude.toFloat() + Coord::epsilon/2) / Coord::scalefactor;
